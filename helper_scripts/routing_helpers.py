...
class RoutingHelpers:
    ...
    def _find_channel_mci(self, channels_list: list, center_freq: float, num_span: float):
        ...
        for channel in channels_list:
            ...
            curr_mci = math.log(abs(curr_mci / (abs(center_freq - curr_freq) - (bandwidth / 2.0))))
    ...
    def _find_link_cost(self, free_channels_dict: dict, taken_channels_dict: dict, num_span: float):
        nli_cost = 0
        num_channels = 0
        for band, free_channels_dicts in free_channels_dict.items():
            for core_num, free_channels_list in free_channels_dicts.items():
                # Update MCI for available channel
                for channel in free_channels_list:
                    if True:# band == 'c' and len(free_channels_dict.keys()) == 1:
                        num_channels += 1
                        ...
                        nli_cost += self._find_channel_mci(channels_list=taken_channels_dict[band][core_num], center_freq=center_freq,
                                                        num_span=num_span)
    ...
    def _find_num_overlapped(self, channel: int, core_num: int, band:str, core_info_dict: dict):
        num_overlapped = 0.0
        if core_num != 6:
    ...
    def find_xt_link_cost(self, free_slots_dict: dict, link_list: list):
        ...
        for band in free_slots_dict:
            for core_num, slots in free_slots_dict[band].items():
                ...
                for channel in slots:
                    ...
                    num_overlapped = self._find_num_overlapped(channel=channel, core_num=core_num, 
                                                               band=band, core_info_dict=core_info_dict)
